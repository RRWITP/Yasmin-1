...
class Activity extends ClientBase
{
    ...
    public function __construct(Client $client, array $activity)
    {
        ...
        $this->assets = (! empty($activity['assets']) ? (new RichPresenceAssets(
            $this->client,
            $this,
            $activity['assets']
        )) : null);
        $this->timestamps = (! empty($activity['timestamps']) ? [
            'start' => (! empty($activity['timestamps']['start']) ? DataHelpers::makeDateTime(
                ((int) (((int) $activity['timestamps']['start']) / 1000))
            ) : null),
            'end'   => (! empty($activity['timestamps']['end']) ? DataHelpers::makeDateTime(
                ((int) (((int) $activity['timestamps']['end']) / 1000))
            ) : null),
        ] : null);
